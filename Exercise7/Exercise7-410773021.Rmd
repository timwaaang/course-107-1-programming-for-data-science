...
最後一行必需能在螢幕顯現此值。
```{r ans11}
unique(transcriptData$學號) %>% length
```
...
```{r ans12}
transcriptData %>% group_by(學號, 學年, 學系, 學屆) %>% summarise(
  學年平均成績=mean(學期成績)
) -> gradeEachYear
gradeEachYear
...
```{r ans13}
gradeEachYear %>% group_by(學系,學屆,學年) %>% mutate(
...
transcriptDataWithSemesterCounts %>% group_by(學號) %>% filter(
  max(第幾學期)>=8,
  between(第幾學期, 1, 8)
) -> filteredTranscriptData
filteredTranscriptData %>% group_by(學號,學系,學屆) %>% summarise(
  學屆平均成績=sum(學期成績*學分數)/sum(學分數)
)->grade4Years
grade4Years %>% group_by(學系,學屆) %>% mutate(
