...
```{r ans12}
library(stringr)
str_subset(names(gsHW2ws), "ans")
```

## 1.3
使用`colMeans(x, na.rm=T)`一口氣計算五題的正確率（產生一個包含5題正確率的數值向量）。
```{r ans13}
gsHW2ws[1:5] %>% colMeans(na.rm=T) -> AA 
```
...
```{r ans17}
which(gsHW2ws[1:5] %>% rowSums(na.rm = T) == 3)
which(gsHW2ws[23:28, c(1:5)] %>% colMeans(na.rm=T) < 0.5)
```
...
allBankData %>% 
  mutate(
    銀行 = as.factor(銀行), 
    `定存利率-三個月-固定` = parse_number(`定存利率-三個月-固定`,locale=cl), 
    `定存利率-一年期-固定` = parse_number(`定存利率-一年期-固定`,locale=cl)
  ) -> allBankData
```
...
cl<-locale(grouping_mark = ",") 
exData %>%　
  mutate_at(vars(2:19), funs(parse_number(.,locale=cl))
  ) -> exData
str_replace(exData$期間, "M", "-") -> exData$期間
str_c(exData$期間, "-01") -> exData$期間
ymd(exData$期間) -> exData$期間
exData
```
...
```{r ans33}
str_sub(colnames(exData)[2:19], end = -5) -> colnames(exData)[2:19]
str_sub(colnames(exData)[2:19], end = -4) -> colnames(exData)[2:19]
exData
```
