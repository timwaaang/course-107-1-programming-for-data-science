...
which(classData$`公-10-06` == 1)->有看公告
which(classData$`公-10-06` == 0)->沒看公告
```
...
library(tidyr)
length(有看公告)->aa
replace_na(有看公告[classData$`練2018-10-17-1` == 1], 0)->bb
length(bb[bb > 0])->cc
cc/aa
```
...
```{r ans14}
length(沒看公告)->dd
replace_na(沒看公告[classData$`練2018-10-17-1` == 1], 0)->ee
length(ee[ee > 0])->ff
ff/dd
```
...
```{r ans23}
difftime(hwData$deadline, hwData$Timestamp, units = "days")->hwData$early
```
