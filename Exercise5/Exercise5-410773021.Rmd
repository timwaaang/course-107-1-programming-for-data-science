...
totalCredits <- function(x){
  transcriptData[transcriptData$學號== x,] %>% 
    mutate(及格=學期成績>=60) %>% 
    summarise(
  學號=x,
  總修習學分數=sum(學分數),
  總實得學分數=sum(學分數[及格])) -> result
  return(result)
}
...
```{r ans16}
transcriptData[transcriptData$`必選修類別（必∕選∕通）`=="通",] ->sample
unique(sample$科目名稱) -> 通識科目總表
```
...
```{r ans18}
transcriptData[transcriptData$科目名稱=="ZT13",] -> ZT13sample
shannon(ZT13sample$系別)
```
