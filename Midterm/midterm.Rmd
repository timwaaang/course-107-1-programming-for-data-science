...
```{r ans133}
sum(isFemale) -> x
str_detect(subsample$課外活動, "團隊活動") %>% sum -> y 
x-y -> z
y/x -> 有參加
z/x -> 沒有參加
...
```{r ans14}
!is.na(hwGrades[2:4]) -> b
rowSums(b) -> hwGrades$交作業次數
hwGrades
```
...
```{r ans16}
hwGrades$學號 %in% classSurvey$學號 -> c
hwGradesSubsample <- hwGrades[c,]
```
...
```{r ans19}
classSurvey$學號[str_detect(subsample$課外活動, "團隊活動")] -> 有團隊的學號
...
```{r ans21}
ymd_hms(hwSubmit$Timestamp, tz="Asia/Taipei") -> hwSubmit$Timestamp
...
```{r ans22}
hwSubmit[1:49,1:4] -> hwSubmit3
difftime("2018-11-06 00:00:00", hwSubmit3$Timestamp, units = "hours") -> hwSubmit3$截止時差
hwSubmit3
```
...
```{r ans23}
mean(hwSubmit3$截止時差[hwSubmit3$女性==T])-mean(hwSubmit3$截止時差[hwSubmit3$女性==F])
```
